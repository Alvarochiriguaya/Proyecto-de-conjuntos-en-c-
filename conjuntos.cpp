...
void difsime(int v, int v2, int c1[], int c2[]);
void limpia();
...
void Principal(){
	...
	HANDLE hConsoleOutput; //windows.h
	COORD coord; 
	hConsoleOutput = GetStdHandle (STD_OUTPUT_HANDLE);
    coord.X = 23; coord.Y = 2;	// definicion de  las coordenadas
	SetConsoleCursorPosition (hConsoleOutput, coord); // indica en donde debo empezar a colocar el texto 
	cout<<"************************************";
	coord.X = 23; coord.Y = 3;  SetConsoleCursorPosition (hConsoleOutput, coord);
	cout<<"*     UNIVERSIDAD DE GUAYAQUIL     *";
	coord.X = 23; coord.Y = 4;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"************************************";
	coord.X = 23; coord.Y = 5;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*  CARRERA INGENIERIA DE SOFTWARE  *";
	coord.X = 23; coord.Y = 6;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"************************************";
	coord.X = 23; coord.Y = 10;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"************************************";
	coord.X = 23; coord.Y = 11;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*      PROYECTO ELABORADO POR:     *";
	coord.X = 23; coord.Y = 12;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"************************************";
	coord.X = 23; coord.Y =13 ;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*      CHIRIGUAYA VELEZ ALVARO     *";
	coord.X = 23; coord.Y = 14;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*       DEL VALLE LINO ANTHONY     *";
	coord.X = 23; coord.Y = 15;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*          HARO PICO SONIA         *";
	coord.X = 23; coord.Y = 16;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*        LOZANO ROBLES PETER       *";
	coord.X = 23; coord.Y = 17;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*      SILVA FAJARDO CRISTOPHER    *";
	coord.X = 23; coord.Y = 18;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"************************************";
	coord.X = 30; coord.Y = 20;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"  CURSO: SOF-MA-1-4  "<<endl<<endl;
	coord.X = 26; coord.Y = 22;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"****************************";
	coord.X = 26; coord.Y = 23;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*                            *";
	coord.X = 26; coord.Y = 24;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*  BIENVENIDOS  AL  PROGRAMA *";
	coord.X = 26; coord.Y = 25;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*       OPERACIONES DE       *";
	coord.X = 26; coord.Y = 26;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*          CONJUNTOS         *";
	coord.X = 26; coord.Y = 27;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"*                            *";
	coord.X = 26; coord.Y = 28;	SetConsoleCursorPosition (hConsoleOutput, coord); 
	cout<<"****************************";
	coord.X = 1; coord.Y = 30; 
...
void menu(int v, int v2, int c1[], int c2[]){
		...
		gotoxy(4,17);printf("5. Diferencia simetrica\n");
		gotoxy(4,18);printf("6. Menu principal");
		gotoxy(4,20);printf("Ingrese opcion a realizar: ");
		scanf("%i",&o);
		if(o==1) unio(v,v2,c1,c2);
		if(o==2) inter(v,v2,c1,c2);
		if(o==3) dif(v,v2,c1,c2);
		if(o==4) com(v,v2,c1,c2);
		if(o==5) difsime(v,v2,c1,c2);
		if(o==6) menuprincipal();
		}while(i);
...
//FUNCION QUE REALIZA EL CÁLCULO PARA LA DIFERENCIA SIMËTRICA DE CONJUNTOS
void difsime(int v, int v2, int c1[], int c2[]){
	limpia();
	int vd=0;
	int ig;
	...
	//LLENA EL VECTOR CON LOS VALORES A-B	
	for(int i=0 ; i<v; i++){
		ig=0;
		for(int j=0; j<v2 && ig==0; j++){
			if(c1[i] == c2[j])
	...
	//LLENA EL VECTOR CON LOS VALORES B-A
	for(int i=0 ; i<v2; i++){
		ig=0;
		for(int j=0; j<v && ig==0; j++){
			if(c2[i] == c1[j])
	...
	int f=40;
	gotoxy(4,11);printf("Diferencia simetrica de conjuntos: {");
	for (int i=0; i<vd; i++){
		gotoxy(f,11);printf("%i, ",d[i]);
		f+=3;
	}
